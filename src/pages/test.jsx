...
const Test = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const billSummaryData = detailResponse.data;
        console.log(billSummaryData)
        ...
        // Extract customer IDs from bill summaries
        const billCustomerIds = billSummaryData.map((bill) => bill.customerId);
        console.log(billCustomerIds)


        // Filter customers whose IDs are present in bill summaries
        ...
        // Combine the Bill Summary with the related Customer Name
  ...
  return (
    <Box bg="pink" w="50vw" h="50vh">
      {billSummaries.map((item, index) => (
        <Text key={index} border="1px solid red">
          {item.invoiceNumber} - {item.customerName} - {item.totalDiscountAmount}
        </Text>
      ))}
    </Box>
  );
